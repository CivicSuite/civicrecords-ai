...
| Current release | v1.4.4 (May 1, 2026) - versions aligned across all files |
...
Status Legend: [IMPLEMENTED] evidenced in code, tests, and routes. [PARTIAL] present but incomplete. [UI SHELL] interface exists without full backend capability. [PLANNED] not implemented. [NEW in v1.1.0] / [NEW in v1.2.0] / [NEW in v1.3.0] / [NEW in v1.4.0] / [UPDATED in v1.4.3] / [UPDATED in v1.4.4] indicate which release introduced or updated a feature.
...
As of v1.4.4, version numbers are aligned across all four authoritative files:
backend/app/config.py: APP_VERSION = "1.4.4"
backend/pyproject.toml: version = "1.4.4"
frontend/package.json: version = "1.4.4"
CHANGELOG.md: [1.4.4] - 2026-05-01
The version drift documented in prior spec versions has been resolved and remains resolved. The CHANGELOG now covers releases through v1.4.4: 0.1.0 (foundation), 1.0.0 (design system + core features), 1.1.0 (department scoping, compliance, and feature sprint), 1.2.0 (Tier 5 installer/onboarding/seeding/model-picker/portal-mode + Tier 6 at-rest encryption ENG-001 closure), 1.3.0 (release-mechanics pass + civiccore v0.1.x integration), 1.4.0 (Phase 2 LLM integration via civiccore v0.2.0 dependency), 1.4.1 (suite-wide civiccore v0.3.0 dependency alignment), 1.4.3 (shared connector-security extraction aligned to civiccore v0.13.0), and 1.4.4 (persisted audit-log hashing/verification aligned to civiccore v0.17.0).
...
As of v1.4.4, the system implements:
...
Central LLM client with context manager, token budgeting, and prompt injection sanitization (Phase 2 LLM integration introduced via civiccore v0.2.0 in v1.4.0; latest published release aligned to civiccore v0.3.0 in v1.4.1; current development line now targets civiccore v0.17.0 for shared search, onboarding, connector host-validation, encrypted-config, ingest, and persisted audit-log helpers)
...
**Release state:** Current release is **v1.4.4** (May 1, 2026), which moves persisted audit-log hashing and verification onto the shared civiccore v0.17.0 line on top of the v1.4.3 connector-security extraction and v1.4.0 Phase 2 LLM integration release. The `[Unreleased]` CHANGELOG block above `[1.4.4]` is the collection point for post-v1.4.4 work.
