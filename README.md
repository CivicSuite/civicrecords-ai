...
**v1.4.5 (May 2, 2026)** — CivicCore sync-consumer patch. Records-AI now consumes civiccore v0.18.1 for live connector retry and circuit-breaker primitives, preserving the existing Records retry semantics and database-backed sync failure workflow while removing reusable local sync-state logic. API URL paths, roles, permissions, and records-side database migrations are unchanged from v1.4.4.
...
- 625 backend + 36 frontend automated tests passing (GitHub Actions CI-verified, run 25246593265 on `1bd3d1c`)
...
- **CI** — 625 backend + 36 frontend tests (CI-verified on `1bd3d1c`, run 25246593265); bootstrap-failure smoke test confirms stack rejects placeholder admin password; `docs/openapi.json` and `frontend/src/generated/api.ts` stale-check enforced on every build (T3D)
