...
export default function Ingestion({ token }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .finally(() => {
        if (!hasError || true) setLoading(false);
      });
  }, [token]);
    ...
    <div>
      ...
      {loading ? (
        <div className="flex items-center justify-center gap-3 py-16 text-gray-500">
          ...
          <span>Loading ingestion data…</span>
        </div>
      ) : (
        <>
          {stats && (
          ...
          <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
            {docs.length === 0 ? (
              <div className="py-12 text-center text-gray-500">
                <p className="font-medium text-gray-700 mb-1">No documents ingested yet.</p>
                <p className="text-sm">
                  Add a data source and run ingestion to see documents here.
                </p>
              </div>
            ) : (
              <table className="w-full text-sm" aria-label="Ingested documents">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    <th className="text-left px-4 py-3 font-medium text-gray-600" scope="col">Filename</th>
                    <th className="text-left px-4 py-3 font-medium text-gray-600" scope="col">Type</th>
                    <th className="text-left px-4 py-3 font-medium text-gray-600" scope="col">Size</th>
                    <th className="text-left px-4 py-3 font-medium text-gray-600" scope="col">Status</th>
                    <th className="text-left px-4 py-3 font-medium text-gray-600" scope="col">Chunks</th>
                    <th className="text-left px-4 py-3 font-medium text-gray-600" scope="col">Ingested</th>
                  </tr>
                </thead>
                <tbody>
                  {docs.map((d) => (
                    <tr key={d.id} className="border-b border-gray-100">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-xs font-medium ${statusColors[d.ingestion_status] || "text-gray-600"}`}
                        >
                          {d.ingestion_status}
                        </span>
                        {d.ingestion_error && (
                          <span
                            className="block text-xs text-red-400 truncate max-w-xs"
                            title={d.ingestion_error}
                          >
                            {d.ingestion_error}
                          </span>
                        )}
