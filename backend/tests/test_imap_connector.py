...
@pytest.mark.asyncio
async def test_ingest_email_source_dispatch(client):
    """_ingest_email_source authenticates, discovers, fetches, and ingests."""
    from unittest.mock import AsyncMock, MagicMock, patch
    from app.ingestion.tasks import _ingest_email_source
    from app.connectors.base import DiscoveredRecord, FetchedDocument
    from datetime import datetime, timezone

    # Build a test email with one safe attachment
    raw_email = _build_email_with_attachments([
        ("budget.pdf", "application/pdf", b"%PDF-1.4 test content here"),
    ])

    # Mock the source object
    source = MagicMock()
    source.id = "00000000-0000-0000-0000-000000000001"
    source.connection_config = {"host": "mail.city.gov", "username": "u", "password": "p"}
    ...
    # extract_safe_attachments is sync — use regular MagicMock
    mock_connector.extract_safe_attachments.return_value = [
        FetchedDocument(
            source_path="attachment://budget.pdf",
            filename="budget.pdf",
            file_type="pdf",
            content=b"%PDF-1.4 test content here",
            file_size=26,
        )
    ]
    ...
    with patch.object(_imap_mod, "ImapEmailConnector", return_value=mock_connector), \
        ...
        mock_ingest.return_value = MagicMock()  # fake Document object

        result = await _ingest_email_source(mock_session, source, user_id=None)

    assert result["discovered"] == 1
    assert result["ingested"] == 2  # email body + 1 attachment
...
@pytest.mark.asyncio
async def test_ingest_email_source_auth_failure(client):
    """_ingest_email_source returns error on auth failure."""
    from unittest.mock import AsyncMock, MagicMock, patch
    from app.ingestion.tasks import _ingest_email_source

    source = MagicMock()
    source.connection_config = {"host": "mail.city.gov", "username": "u", "password": "bad"}
    ...
    assert result["error"] == "IMAP authentication failed"
